...
import Java.utilitaires.CoordonneesStation;
...
public class JCDecauxRequest {
    ...
    public static ArrayList<CoordonneesStation> extraire3Stations(ArrayList<CoordonneesStation>coordonnees,JsonArray jArray){
        ArrayList<CoordonneesStation> result = new ArrayList<CoordonneesStation>();
        for(int i=0;i<coordonnees.size();++i)
        {
            coordonnees.get(i).setDistance(Float.parseFloat(jArray.getJsonNumber(i).toString()));
        }
        Collections.sort(coordonnees);
        for(int i = 0;i<3;++i)
        {
            result.add(coordonnees.get(i));
        }
        return result;
    }
    public static  void main(String [] args){
        Coordonnees moi = new Coordonnees(43.55698210000001,1.4665253000000575);
       // ArrayList<main.java.CoordonneesStation> a = new ArrayList<CoordonneesStation>();
      //  a.add(new CoordonneesStation(2,43.500,1.444));
       // a.add(new CoordonneesStation(3,43.604,1.395));
        ArrayList<CoordonneesStation> a = getStations();
    }
}
